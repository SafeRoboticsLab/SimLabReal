...
"""A class for training posterior of PAC-Shield-Both.

This file implements the training of the posterior policy with
latent-conditioned performance and backup policies, which is the method
...
class PolicyPosteriorBothLatent(BaseTraining):
    ...
    def learn(
        ...
        """Learns the posterior of PAC-Shield-Both.
        ...
        train_records = []
        train_progress = [[], [], [], []]  # perf, backup, shield, kl/renyi
        violation_record = []
        episode_record = []
        ...
        if plot_figure or save_figure:
            self.get_figures(
                ...
                figure_folder_backup=figure_folder_backup, plot_shield=True,
        ...
        while self.cnt_step <= max_steps:
            ...
            shield_flag, _ = check_shielding(
                ...
                context_backup=z_backup, state=_states,
            ...
            for env_ind, (s_, r, done, info) in enumerate(
                ...
                # Saves extra info: a_perf, z_backup and append
            ...
            if (
                ...
                # Updates latent and perf critic
                ...
                if cnt_opt % check_opt_freq == 0:
                    ...
                    progress_backup = self.backup.check(
                        ...
                        revert_task=True,
                        # kwargs for env.simulate_trajectories
                        latent_dist=self.backup.latent_ps,
                        fixed_init=True
                    )
                    progress_perf_shield_value = self.performance.check(
                        env=venv,
            ...
            # Updates gamma, lr etc. Backup does not have any to be updated.
