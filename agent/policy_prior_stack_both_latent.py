...
"""A class for training prior of PAC-Shield-Both with frame stacking.

This file implements the training of the prior policy with latent-conditioned
performance and latent-conditioned backup policies, which is the method
...
(observation-marginal MI) implemented in SAC_maxEnt.py. The backup policy is
trained with discounted safety Bellman equation and diversity reward based on
either observation-marginal MI or observation-conditional MI implemented in
SAC_dads_prior.py.
"""
...
class PolicyPriorStackBothLatent(BaseTrainingStack):
    ...
    def __init__(
        ...
        self.backup = SAC_dads_prior(
            cfg_backup.train, cfg_backup.arch, cfg_env
        )
    ...
    def learn(
        ...
        """Learns the prior of PAC-Shield-Both with frame stacking.
        ...
        z_perf = self.performance.latent_prior.sample((self.n_envs,))
        z_perf = z_perf.to(self.device)
        z_backup = self.backup.latent_prior.sample((self.n_envs,))
        ...
        while self.cnt_step <= max_steps:
            ...
            with torch.no_grad():
                ...
                a_perf, _ = self.performance.actor.sample(
                    ...
                    latent=z_perf[use_perf]
                )

                # latents for backup are sampled at every step
                # z_backup = self.backup.latent_prior.sample((self.n_envs,))
                # z_backup = z_backup.to(self.device)
                a_backup, _ = self.backup.actor.sample(
                    _prev_obs_stack[use_backup], append=append_all[use_backup],
                    latent=z_backup[use_backup]
                )
            ...
            if apply_shielding:
                shield_flag, _ = check_shielding(
                    ...
                    append_all, context_backup=z_backup, state=_prev_states,
            ...
            for env_ind, (_, r, done, info) in enumerate(
                ...
                if done:
                    ...
                    # Re-sample z
                    z_perf[env_ind] = self.performance.latent_prior.sample(
                    ).to(self.device)
                    z_backup[env_ind] = self.backup.latent_prior.sample().to(
                        self.device
                    )
            ...
            if (
                ...
                for timer in range(num_update_per_opt):
                    ...
                    batch_backup = self.unpack_batch(
                        _batch, _batch_nxt, get_perf_action=False,
                        get_latent_backup=True, get_latent=True
                    )
                ...
                if cnt_opt % check_opt_freq == 0:
                    ...
                    progress_backup = self.backup.check(
                        ...
                        sample_task=True,
                        num_rnd_traj=num_rnd_traj,
                        latent_dist=self.backup.latent_prior,
                        traj_size=self.traj_size,
            ...
            for _ in range(self.n_envs):
                ...
                if self.backup.latent_std_scheduler is not None:
                    self.backup.latent_std_scheduler.step()
