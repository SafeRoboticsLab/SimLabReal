...
def main(cfg_file, cfg):
    ...
    if cfg.agent.use_wandb:
        ...
        if cfg.agent.name == 'PolicyPosteriorPerfLatent':
            wandb.config.update({
                'BOUND_TYPE': cfg.performance.train.bound_type,
                'DIV_WEIGHT': cfg.performance.train.div_weight
            })
        elif cfg.agent.name == 'PolicyPosteriorBothLatent':
            wandb.config.update({
                'BOUND_TYPE': cfg.performance.train.bound_type,
                'DIV_WEIGHT': cfg.performance.train.div_weight,
                'BOUND_TYPE (B)': cfg.backup.train.bound_type,
                'DIV_WEIGHT (B)': cfg.backup.train.div_weight
            })
