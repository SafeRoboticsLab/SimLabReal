'''This file is directly taken from https://github.com/HusseinBakri/BlenderPythonDecimator.
...
Usage: blender -b -P blender_simplify.py -- --ratio 0.5 --inm 'Original_Mesh.obj' --outm 'Output_Mesh.obj'
...
def get_args():
    ...
    parser.add_argument(
        '-r', '--ratio',
        help="Ratio of reduction, Example: 0.5 mean half number of faces "
    )
...
for i, obj in enumerate(meshes):
    ...
    print(
        "{} has {} verts, {} edges, {} polys".format(
            obj.name, len(obj.data.vertices), len(obj.data.edges),
    ...
    print(
        "{} has {} verts, {} edges, {} polys after decimation".format(
            obj.name, len(obj.data.vertices), len(obj.data.edges),
            len(obj.data.polygons)
        )
    )
