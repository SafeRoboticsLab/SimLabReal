...
class Encoder(nn.Module):
	def __init__(self,
		...
		self.conv_1 = nn.Sequential(  # Nx1x128x128
            nn.Conv2d(in_channels=input_num_chann, out_channels=out_cnn_dim//4, 
                kernel_size=5, stride=1, padding=2),
			nn.ReLU()
            )    # Nx16x128x128

		self.conv_2 = nn.Sequential(
            nn.Conv2d(in_channels=out_cnn_dim//4, out_channels=out_cnn_dim//2, 
                kernel_size=3, stride=1, padding=1),
			nn.ReLU()
            )    # Nx32x128x128

		# Spatial softmax, output 64 (32 features x 2d pos)
		self.sm = SpatialSoftmax(height=img_size, width=img_size, 
            channel=out_cnn_dim//2)

		# MLP
		self.linear_1 = nn.Sequential(
            nn.Linear(out_cnn_dim+dim_mlp_append, out_cnn_dim*2, bias=True),
            nn.ReLU()
            )

		self.linear_2 = nn.Sequential(
            nn.Linear(out_cnn_dim*2, out_cnn_dim*2, bias=True),
            nn.ReLU()
            )

		# Output action
		self.linear_out = nn.Linear(out_cnn_dim*2, z_total_dim*2, bias=True)
