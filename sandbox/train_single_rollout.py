...
from maml_rl.metalearners import TRPO
...
def main(args):
	...
	train_tasks_all = sampler.sample_tasks(num_tasks=config['num-train-task'], uniform=False)	# should not uniform sampling since we do not know the distribution for PAC-Bayes
	...
	for batch in trange(config['num-batches']):
		...
		task_ind_chosen = np.random.choice(100, size=config['meta-batch-size'], replace=False).astype('int')
		...
		if batch % 10 == 0 and batch > 0:
			...
			obs = plt.Circle(task['obs_loc'], task['obs_radius'], color='r')
			obs_buffer = plt.Circle(task['obs_loc'], task['obs_radius']+0.1, color='b')
			plt.gca().add_patch(obs_buffer)
			plt.gca().add_patch(obs)
			for episode in valid_episodes:	# plot all 
				plt.plot(episode.observations[:,0:2,0], episode.observations[:,0:2,1], '.-', markersize=1, lw=0.5)
