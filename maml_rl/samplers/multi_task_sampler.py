...
class MultiTaskSampler(Sampler):
	...
	def sample_tasks(self, num_tasks, uniform=False):
		return self.env.unwrapped.sample_tasks(num_tasks, uniform)
...
class SamplerWorker(mp.Process):
	def __init__(self,
		...
		self.envs = SyncVectorEnv(env_fns,	#* parallel
	...
	def sample(self,
			   index,
			   latent_params=None,# for fine-tuning, latent_params are provided
			   num_steps=1,
			   fast_lr=0.5,
			   gamma=0.95,
			   gae_lambda=1.0,
		...
		if self.latent_size > 0 and latent_params is None:
			...
			# Latent distribution for log likelihood
			latent_dist = MultivariateNormal(mu, torch.diag(torch.exp(logvar)))

			# Sample
			latent = latent_dist.sample(sample_shape=(self.batch_size,))
			...
			latent_params['mu'].requires_grad = True
			latent_params['logvar'].requires_grad = True
		...
		for step in range(num_steps):
			train_episodes = self.create_episodes(latent=latent,
												  nn_params=nn_params,
												  gamma=gamma,
												  gae_lambda=gae_lambda,
												  device=device)
  			
     		# save latent (detached) with episodes
			if self.latent_size > 0:
				train_episodes.latent = latent.clone().detach()
			...
			with self.policy_lock:
				if self.latent_size > 0:
					...
					latent = latent.unsqueeze(0).repeat(num_step,1,1)

					loss = reinforce_latent_loss(train_episodes, 
                                  				latent=latent, 
				...
				latent_params, nn_params = self.policy.update_params(loss,
		...
		# Resample latent using updated policies
		if self.latent_size > 0:
			latent_dist = MultivariateNormal(latent_params['mu'],
                            	torch.diag(torch.exp(latent_params['logvar'])))
			latent = latent_dist.sample(sample_shape=(self.batch_size, ))
		else:
			latent = None

		# Sample the validation trajectories with the adapted policy
		valid_episodes = self.create_episodes(latent=latent,
											  nn_params=nn_params,
											  gamma=gamma,
											  gae_lambda=gae_lambda,
											  device=device)
		
  		# save latent (detched) with episodes
		if self.latent_size > 0:
	...
	def create_episodes(self,
		...
		t0 = time.time()
		for item in self.sample_trajectories(latent=latent, params=nn_params):
	...
	def sample_trajectories(self, latent, params=None):
		observations = self.envs.reset()
		with torch.no_grad():
			while not self.envs.dones.all():
				observations_tensor = torch.from_numpy(observations)

				#* make sure number of trajectories match the latent
				num_traj = observations_tensor.shape[0]
				if latent is not None:
					latent = latent[:num_traj]
				pi = self.policy(observations_tensor, params=params, latent=latent)
